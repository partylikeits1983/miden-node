...
fn main() -> miette::Result<()> {
    ...
    build_tonic_from_descriptor(proving_service_descriptor.clone(), std_path, true)?;

    // Build nostd version
    let nostd_path = format!("{GENERATED_OUT_DIR}/nostd");
    build_tonic_from_descriptor(proving_service_descriptor, nostd_path.clone(), false)?;
